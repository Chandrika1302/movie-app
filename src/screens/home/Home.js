                ...
                <div className="flex-container">
                    <div className="left">
                        <GridList cellHeight={350} cols={4} className={classes.gridListMain}>
                            {moviesData.map(movie => (
                                <GridListTile className="released-movie-grid-item" key={"grid" + movie.imdbID} onClick={() => this.movieClickHandler(movie.imdbID)}>
                                    <img src={movie.Poster} className="movie-poster" alt={movie.Title} />
                                    <GridListTileBar
                                        title={movie.Title}
                                        subtitle={<span>Release Date: {new Date(movie.Year).toDateString()}</span>}
                                    />
                                </GridListTile>
                            ))}
                        </GridList>
